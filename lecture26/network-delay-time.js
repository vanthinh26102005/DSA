var networkDelayTime = function (times, n, k) {
    ...
    const queue = [initNodeIdx, layerEnd] // init the queue
    const seen = new Set([initNodeIdx]) // keep track of seen nodes
    let paths = [[0, initNodeIdx]] // the first num (0) is total travel
    ...
    while (queue.length > 0) {
        ...
        for (let i = 0; i < curNode.length; i++) {
            ...
            if (!seen.has(neighborNodeIdx)) {
                ...
                // update path with this new neighbor + travel tmie to this neighbor
